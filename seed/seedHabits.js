...
// Убедись, что путь до модели Habit верный (обычно на уровень выше)
const Habit = require('../models/Habit'); 

const mongoUri = process.env.MONGO_URI;

if (!mongoUri) {
  console.error('Ошибка: Переменная MONGO_URI не найдена в окружении!');
  process.exit(1);
}
...
const seedDB = async () => {
  try {
    ...
    process.exit(0); // Обязательно выходим
  } catch (err) {
